...
/**
 * Persistir as ofertas na tabela `GEC.ISRH_OFERTA_GAS`
 * - Realiza a mutação das `ofertas`
 * @param   { string } casoMoagem
 * @param   { import("../../util/types.js").IOferta[] } ofertas
 * @param   { Database } conexao
 * @returns { Promise<number> } linhas afetadas
 */
async function persistir_dados (casoMoagem, ofertas, conexao) {
    if (ofertas.length == 0) return 0

    let datetime = json_datetime(-3, false).replace("-03:00", "")
    for (let oferta of ofertas) {
        oferta.agora = datetime
        oferta.casoMoagem = casoMoagem
        oferta.u_volume = parseFloat(oferta.u_volume)
    }

    let sql = await conexao.execute_many(
        `INSERT INTO GEC.ISRH_OFERTA_GAS 
            (ISCI_DF_CASO_MOAG, ISOG_CD_PONTO_OFERTA_GAS, ISOG_CD_FAIXA_OFERTA_GAS, ISOG_DF_OFERTA_GAS, ISOG_MD_VOLUME_OFERTA_GAS, ISOG_DF_GRAVACAO_REGISTRO) 
        VALUES 
            (TO_DATE(:casoMoagem, 'dd-MM-yyyy hh24:mi'), :u_offer, :u_offer_range, TO_DATE(:u_date, 'yyyy-MM-dd'), :u_volume, TO_DATE(:agora, 'yyyy-MM-dd"T"hh24:mi:ss'))`,
        ofertas
    )
 ...
 * @returns { Promise<{ next: boolean, ofertas: import("../../util/types.js").IOferta[] }> } retorna as ofertas e se há próxima página
 */
async function obter_dados_tabela (grouping, offset) {
    let querys = new URLSearchParams({
        ...
        "sysparm_fields": "u_volume,u_date,u_offer_range,u_offer",
    }), 
    headers = {
        "Authorization": ENV.gec_controle_basic_auth,
        "apikey": ENV.gec_controle_apikey,
    ...
    response = await send_request(
        `https://${ ENV.gec_controle_host }:443/ge/pre-operacao/v1/controle/x_petro_gas_energy_moagen_maximum_offer`, 
        "GET", querys, headers
    )
...
/**
 * Fluxo de carga da tabela `x_petro_gas_energy_moagen_maximum_offer`
 * - 1 - Informar o início na tabela GEC de controle
 * - 2 - Obter dados da tabela via API do Service Now
 * - 3 - Persistir os dados na tabela `GEC.ISRH_OFERTA_GAS`
...
export default async (body, context) => {
     ...
     */
    let conexao
    console.info(`id(${ context.invocationId }) | retry(${ JSON.stringify(context.retry) }) | Inicio do fluxo oferta`)
    ...
    try {
        ...
        while (proximaPagina) {
            let dados = await obter_dados_tabela(body.casoMoagem, offset),
                linhasAfetadas = await persistir_dados(body.casoMoagem, dados.ofertas, conexao)
            
            proximaPagina = dados.next
            offset += dados.ofertas.length
        ...
        console.info(`id(${ context.invocationId }) | retry(${ JSON.stringify(context.retry) }) | Fim do fluxo oferta`)

    } catch(erro) {
        console.error(`id(${ context.invocationId }) | retry(${ JSON.stringify(context.retry) }) | erro(${ erro?.message }) | Erro inesperado no fluxo oferta`)
        
        await conexao.rollback()
        await controle_atualizacao_gec(conexao, body, 0, "error", `id(${ context.invocationId }) | Falha na consulta dos dados ou na persistência da tabela de oferta.`)
        await conexao.commit()
        await conexao.close()
